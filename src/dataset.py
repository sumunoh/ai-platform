...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.set_type == 'train':
            ...
            target = self.hdf5.train_target(self.num, self.index[index])
        elif self.set_type == 'valid':
            input = self.hdf5.valid_input(self.num, self.index[index])
            target = self.hdf5.valid_target(self.num, self.index[index])
        elif self.set_type == 'test':
            input = self.hdf5.test_input(self.num, self.index[index])
            target = self.hdf5.test_target(self.num, self.index[index])
